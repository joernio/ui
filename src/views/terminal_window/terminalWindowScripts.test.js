...
describe("function constructInputToWrite: ", ()=>{
  ...
  it("expect function to return expected value: ", ()=>{
    const expected = TV.clearLine + 
                    TV.joernDefaultPrompt + 
                    TWS.data_obj.data + 
                    TV.carriageReturn + 
                    TV.cursorPositionFromStart.split('<n>')
...
describe("function constructOutputToWrite: ", ()=>{
  ...
  it("expect function to return '<pre>{value}<pre>'", ()=>{
    expect(TWS.constructOutputToWrite(null, "test", true)).toBe('<pre>test</pre>');
  });

  it("expect function to return specific string configuration for each combination: ", ()=>{
    expect(TWS.constructOutputToWrite(null, null)).toBe(TV.clearLine + " Running script .....");
    expect(TWS.constructOutputToWrite("test ", null)).toBe(TV.clearLine + "test Running script .....");
    expect(TWS.constructOutputToWrite(null, "output")).toBe(TV.clearLine + " output");
    expect(TWS.constructOutputToWrite("test ", "output")).toBe(TV.clearLine + "test output");
  });
...
describe("function handleEmptyWorkspace: ", ()=>{
    ...
    it("expect function to return {isMaximized: true}: ", ()=>{
       expect(TWS.handleEmptyWorkspace({projects: {}},{projects: {}}).isMaximized).toBe(true);
    });

    it("expect function to return {isMaximized: false}: ", ()=>{
        expect(TWS.handleEmptyWorkspace({projects: {"a": {}}},{projects: {}}).isMaximized).toBe(false);
     });
});
...
describe("function openXTerm: ", ()=>{
    ...
    beforeEach(()=>{
        ...
        open = jest.fn();
        term = {onKey,open};
        handleXTermOnKey = jest.spyOn(TWS, 'handleXTermOnKey').mockImplementation(()=>new Promise(r=>r()));
    });
...
describe("function removeOldestQueryFromHistory: ", ()=>{
    it("expect function to delete first property of history.prev_queries object: ", ()=>{
        ...
        const response = TWS.removeOldestQueryFromHistory(history, Object.keys(history));
...
describe("function addQueryToHistory: ", ()=>{
    ...
    beforeEach(()=>{
     getNext = jest.spyOn(TWS, "rotateNext").mockImplementation(history=>{
         history.prev_queries = {...history.prev_queries, ...history.next_queries};
    ...
    it("expect prev_queries to be a combination of prev_queries, next_queries and queue: ", ()=>{
       const history = {prev_queries: {a:{"a":true}, b: {"b":true}}, next_queries : {c: {"c": true}, d: {"d": true}}};
       const queue = {"e":{"e": true}};

       const response = TWS.addQueryToHistory(history, queue, Object.keys(queue)[0]);

       expect(JSON.stringify(response)).toBe(
           JSON.stringify({prev_queries: {
...
describe("function rotateNext: ", ()=>{
  it("expect last history in history.next_queries to be rotated into history.prev_queries: ", ()=>{
    const history = {prev_queries: {a:{"a":true}, b: {"b":true}}, next_queries : {c: {"c": true}, d: {"d": true}}};
    const rotated = {prev_queries: {a:{"a":true}, b: {"b":true}, d: {"d": true}}, next_queries : {c: {"c": true}}};
  ...
  it("expect no rotation: ", ()=>{
    const history = {prev_queries: {a:{"a":true}, b: {"b":true}}, next_queries : {}};
    const response = TWS.rotateNext(history);
...
describe("function rotatePrev: ", ()=>{
   it("expect last history in history.prev_queries to be rotated into history.next_queries: ",  ()=>{
    const history = {prev_queries: {a:{"a":true}, b: {"b":true}}, next_queries : {c: {"c": true}, d: {"d": true}}};
    const rotated = {prev_queries: {a:{"a":true}}, next_queries : {c: {"c": true}, d: {"d": true}, b: {"b":true}}};
   ...
   it("expect no rotation: ", ()=>{
    const history = {prev_queries: {}, next_queries : {c: {"c": true}, d: {"d": true}}};
    const response = TWS.rotatePrev(history);
...
describe("function initFitAddon: ", ()=>{
   ...
   it("expect function to return null: ", ()=>{
    [undefined, null, "", false, 0].map(arg=>expect(TWS.initFitAddon(arg)).toBe(null));
   });
...
describe("function handlePasteFromClipBoard: ", ()=>{
    ...
    beforeEach(()=>{
       ...
       updateCursorPosition = jest.spyOn(TWS, "updateCursorPosition").mockImplementation(()=>{});
       termWrite = jest.spyOn(TWS, "termWrite").mockImplementation(()=>new Promise(r=>r()));
       constructInputToWrite = jest.spyOn(TWS, "constructInputToWrite").mockImplementation(()=>"input-to-write");
       handleWriteToCircuitUIInput = jest.spyOn(TWS, "handleWriteToCircuitUIInput").mockImplementation(()=>{});
    ...
    it("expect function to call specific functions: ",  async ()=>{
       ...
       expect(windowActionApi.registerPasteFromClipBoardListener).toHaveBeenCalled();
...
describe("function handleEnter: ", ()=>{
    ...
    beforeEach(()=>{
      ...
      updateData = jest.spyOn(TWS, "updateData").mockImplementation(()=>{});
      updateCursorPosition = jest.spyOn(TWS, "updateCursorPosition").mockImplementation(()=>{});
      termWriteLn = jest.spyOn(TWS, "termWriteLn").mockImplementation(()=>new Promise(r=>r()));
      constructOutputToWrite = jest.spyOn(TWS, "constructOutputToWrite").mockImplementation((a,b, c)=>`${a}-${b}-${c}`);
      handleWriteToCircuitUIResponse = jest.spyOn(TWS, "handleWriteToCircuitUIResponse").mockImplementation(()=>{});
      handleWriteToCircuitUIInput = jest.spyOn(TWS, "handleWriteToCircuitUIInput").mockImplementation(()=>{});
    });
    ...
    it("expect function to call specific functions: ",  async()=>{
      ...
      expect(constructOutputToWrite).toHaveBeenCalledWith(null, TWS.data_obj.data, true);
      expect(handleWriteToCircuitUIResponse).toHaveBeenCalledWith(refs, "null-test-true", "query");
...
describe("function handleBackspace: ", ()=>{
  ...
  beforeEach(()=>{
    ...
    updateCursorPosition = jest.spyOn(TWS, "updateCursorPosition").mockImplementation(()=>{});
    termWrite = jest.spyOn(TWS, "termWrite").mockImplementation(()=>new Promise(r=>r()));
    constructInputToWrite = jest.spyOn(TWS, "constructInputToWrite").mockImplementation(()=>"tes");
    handleWriteToCircuitUIInput = jest.spyOn(TWS, "handleWriteToCircuitUIInput").mockImplementation(()=>{});
  });
...
describe("function handleArrowUp: ", ()=>{
  ...
  beforeEach(()=>{
    ...
    getPrev = jest.spyOn(TWS, "getPrev").mockImplementation(({prev_queries})=>prev_queries[Object.keys(prev_queries)[Object.keys(prev_queries).length - 1]]);
    rotatePrev = jest.spyOn(TWS, "rotatePrev").mockImplementation(obj=>obj);
    updateData = jest.spyOn(TWS, "updateData").mockImplementation(()=>{});
    updateCursorPosition = jest.spyOn(TWS, "updateCursorPosition").mockImplementation(()=>{});
    termWrite = jest.spyOn(TWS, "termWrite").mockImplementation(()=>new Promise(r=>r()));
    constructInputToWrite = jest.spyOn(TWS, "constructInputToWrite").mockImplementation(()=>"test");
    handleWriteToCircuitUIInput = jest.spyOn(TWS, "handleWriteToCircuitUIInput").mockImplementation(()=>{});
  });

  it("expect function to call specific functions: ",  async()=>{
    const term = {};
    const refs = {circuitUIRef: {current: {children: [null,{children:[{value: null}]}]}}};
    const history = {prev_queries: {a:{"query":"a"}, b: {"query":"b"}}, next_queries : {c: {"query":"c"}, d: {"query":"d"}}};
...
describe("function handleArrowDown: ", ()=>{
  ...
  beforeEach(()=>{
    ...
    TWS.data_obj.data = '';
    ev = {preventDefault: jest.fn()};
    getNext = jest.spyOn(TWS, "getNext").mockImplementation(({next_queries})=>next_queries[Object.keys(next_queries)[Object.keys(next_queries).length - 1]]);
...
describe("function handleWriteQueryResult: ", ()=>{
  ...
  it("expect function to call specific functions: ",  async()=>{
    ...
    const refs = {};
    const latest = {result: {stdout:"stdout1\nstdout2\nstdout3", stderr: "stderr1\nstderr1\nstderr1"}};
    ...
    latest.result.stdout.split("\n").forEach((line, index)=>{
      expect(constructOutputToWrite).toHaveBeenNthCalledWith(index + 1, null, line);
      expect(termWriteLn).toHaveBeenNthCalledWith(index + 1, term, `null-${line}-undefined`);
    });

    expect(constructOutputToWrite).toHaveBeenNthCalledWith(4, null, latest.result.stdout, true );
    expect(handleWriteToCircuitUIResponse).toHaveBeenCalledWith(refs, `null-${latest.result.stdout}-true`, "stdout");
 ...
 it("expect function to output stderr value: ", async ()=>{
  ...
  expect(constructOutputToWrite).toHaveBeenNthCalledWith(4, null, latest.result.stderr, true );
  expect(handleWriteToCircuitUIResponse).toHaveBeenCalledWith(refs, `null-${latest.result.stderr}-true`, "stderr");
  expect(term.prompt).toHaveBeenCalled();
  expect(setTerminalBusy).toHaveBeenCalledWith(false);
  expect(store.dispatch).toHaveBeenCalledWith(false);
 });
...
describe("function handleWriteScriptQuery: ", ()=>{
  ...
  it("expect function to call specific functions: ",  async()=>{
    ...
    expect(constructOutputToWrite).toHaveBeenNthCalledWith(2, null, "Running script .....", true);
    expect(handleWriteToCircuitUIResponse).toHaveBeenCalledWith(refs, "null-Running script .....-true", "query");
...
describe("function handleWriteQuery: ", ()=>{
  ...
  it("expect function to call specific functions: ",  async()=>{
    ...
    latest.query.split("\n").forEach((line, index)=>{
      if(index < 1){
        expect(constructOutputToWrite).toHaveBeenNthCalledWith(index + 1, TV.joernDefaultPrompt, line);
        expect(termWriteLn).toHaveBeenNthCalledWith(index + 1, term, `${TV.joernDefaultPrompt}-${line}-undefined`);
      }else{
        expect(constructOutputToWrite).toHaveBeenNthCalledWith(index + 1, null, line);
        expect(termWriteLn).toHaveBeenNthCalledWith(index + 1, term, `null-${line}-undefined`);
      }
    ...
    expect(constructOutputToWrite).toHaveBeenNthCalledWith(4, null, latest.query, true );
    expect(handleWriteToCircuitUIResponse).toHaveBeenCalledWith(refs, `null-${latest.query}-true`, "query");
    expect(setTerminalBusy).toHaveBeenCalledWith(true);
    expect(store.dispatch).toHaveBeenCalledWith(true);
 });
