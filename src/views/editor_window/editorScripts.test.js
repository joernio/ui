...
describe("function HandleFileAddedToRecent: ",()=>{
    let readRecentFile,
        shouldGoToLine,
        goToLine,
        highlightRange,
   ...
   beforeEach(async()=>{
    ...
    readRecentFile = jest.spyOn(editorScripts, 'readRecentFile').mockImplementation(path=>new Promise((res, rej)=>{
           if (path && path !== path.split('/')[path.split('/').length - 1]) {
    ...
    shouldGoToLine = jest.spyOn(editorScripts, 'shouldGoToLine').mockImplementation(()=>({startLine: 0, endLine: 0}));
    goToLine = jest.spyOn(editorScripts, 'goToLine').mockImplementation(()=>{});
    highlightRange = jest.spyOn(editorScripts, 'highlightRange').mockImplementation(()=>{});
      ...
      response = await editorScripts.handleFileAddedToRecent(refs, props);
      responseTwo = await editorScripts.handleFileAddedToRecent(refs, propsTwo);
      responseThree = await editorScripts.handleFileAddedToRecent(refs, propsThree);
      responseFour = await editorScripts.handleFileAddedToRecent(refs, propsFour);
    Promise.resolve().then(() => jest.runAllTimers());

     
   });

   it("expect response and responseTwo to be an object with obj.openFileContent = '' and obj.isReadOnly = false ", ()=>{
     expect(response.openFileContent).toBe('');
     expect(response.isReadOnly).toBe(false);
     expect(responseTwo.openFileContent).toBe('');
     expect(responseTwo.isReadOnly).toBe(false);
   });

   it("expect responseThree to be an object with obj.openFileContent = 'abcdefghijklmnopqrstuvwxyz' and obj.isReadOnly = ", ()=>{
     expect(responseThree.openFileContent).toBe('abcdefghijklmnopqrstuvwxyz');
     expect(responseThree.isReadOnly).toBe(true);
     expect(responseFour.openFileContent).toBe('abcdefghijklmnopqrstuvwxyz');
     expect(responseFour.isReadOnly).toBe(false);
   });

   it('expect readRecentFile to have been called',()=>{
    expect(readRecentFile).toHaveBeenCalled();
    });
   ...
   it('expect goToLine to have been called',()=>{
        expect(goToLine).toHaveBeenCalled();
    });
...
describe("function highlightRange: ", ()=>{
   let delta_decorations,
       ...
       beforeEach(()=>{
        ...
        editor = {
            deltaDecorations: (arr1, arr2)=>{
                delta_decorations = [{...(arr1[0] ? arr1[0] : {}), ...(arr2[0] ? arr2[0] : {})}];
       ...
       it("expect delta_decorations.range.res to be [0,0,0,0]",()=>{
          ...
          expect(JSON.stringify(delta_decorations[0].range.res)).toBe("[0,0,0,0]");
       });
       ...
       it("expect delta_decorations.range.res to be [2,0,2,0]",()=>{
        ...
        range.endLine = 2;
        editorScripts.highlightRange(editor, range);
        expect(JSON.stringify(delta_decorations[0].range.res)).toBe("[2,0,2,0]");
        ...
        range.endLine = 2;
        editorScripts.highlightRange(editor, range);
        expect(delta_decorations[0].options.inlineClassName).toBe("editor-line-highlight");
        ...
        it("expect delta_decorations.range.res to be [1,0,2,0]",()=>{
            ...
            range.endLine = 2;
            editorScripts.highlightRange(editor, range);
            expect(JSON.stringify(delta_decorations[0].range.res)).toBe("[1,0,2,0]");
         });
...
describe("function readRecentFile: ", ()=>{
  it("expect readRecentFile(null) to reject", async()=>{
      ...
      const response = await editorScripts.readRecentFile(null)
                       .then(data=>resolved)
                       .catch(err=>rejected);
  ...
  it("expect readRecentFile('a') to reject", async()=>{
    const resolved = "resolved";
    const rejected = "rejected";
    const response = await editorScripts.readRecentFile("a")
                            .then(data=>resolved)
                            .catch(err=>rejected);
  ...
  it("expect readRecentFile('/a/b/c/d') to resolve", async()=>{
    const resolved = "resolved";
    const rejected = "rejected";
    const response = await editorScripts.readRecentFile('/a/b/c/d')
                            .then(data=>resolved)
                            .catch(err=>rejected);
...
describe("function shouldGoToLine: ", ()=>{
  let props = {files: {recent: {}}, query: {results: true}};
  beforeEach(()=>{
    jest.spyOn(scripts, "isFilePathInQueryResult").mockImplementation(bool=> bool ? "/a/b/c/d/e/f" : false);
    jest.spyOn(editorScripts, "isLineNumberInQueryResult").mockImplementation(()=>({ startLine: 4, endLine: 11 }));
  });
