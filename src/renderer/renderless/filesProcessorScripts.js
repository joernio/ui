...
import { vars as binaryViewerVars, vars } from '../components/binary_viewer/binaryViewerScripts';
...
export const processFiles = async props => {
  ...
  for(const path of paths_to_close){
    await closeFile(path);
  };

  if(latest?.query?.startsWith("importCode.ghidra") && latest.result?.stdout){// if the latest result is an import of a binary project, clear binaryViewerCache
    store.dispatch(setBinaryViewerCache({filePath: '',selectedMethodIndex: null, selectedMethodBinary: "", methods: []}))
  };
...
export const processMethodListForBinaryProjects = circuit_ui_responses => {
  ...
  if(!activeProject?.language || activeProject.language !== joernBinaryLanguage) return;
    ...
    if(item?.res_type === "query" &&
     circuit_ui_responses.all[item.block_id].ui_query.value.includes("cpg.method.l")){
      ...
      store.dispatch(setBinaryViewerCache({
          filePath,
    ...
    } else if(item?.res_type === "stdout" && circuit_ui_responses.all[item.block_id].ui_query.value.includes("cpg.method.l") ){

      const value = circuit_ui_responses.all[item.block_id]["ui_response"]["responses"][item.sub_block_id].value;
      const method = {
        index: binaryViewerVars.tempMethods.length,
        value: deepClone(value)
      };

      binaryViewerVars.tempMethods = [...binaryViewerVars.tempMethods, method];
      if(!binaryViewerVars.tempMethodsNameIndexMapping[value.name]){
        binaryViewerVars.tempMethodsNameIndexMapping[value.name] = method.index;
      };
      ...
      if(binaryViewerVars.debouncedSetBinaryViewerCache){
        ...
        binaryViewerVars.debouncedSetBinaryViewerCache({
          ...
          methodsNameIndexMapping: binaryViewerVars.tempMethodsNameIndexMapping});
      };
    } else if(item?.res_type === "stdout" && circuit_ui_responses.all[item.block_id].ui_query.value.split('cpg.method.name("')[1]?.split('").call.code.p')[0]){

      const method_name = circuit_ui_responses.all[item.block_id].ui_query.value.split('cpg.method.name("')[1].split('").call.code.p')[0];
      const value = circuit_ui_responses.all[item.block_id]["ui_response"]["responses"][item.sub_block_id].value;
      let cleaned_value = null;

      if(!cleaned_value){
        cleaned_value = value.split(/List\(\n *"/)[1]?.split('"\n)\n</pre>')[0]?.replaceAll(/",\n *"/g,'\n');
      };
      ...
      if(method_name && cleaned_value){
        const {binaryViewerCache} = store.getState().files;
        const index = binaryViewerCache.methodsNameIndexMapping[method_name];
