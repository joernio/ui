export const resizeHandler = (drawerWidth, diff, explorerWindowEl) => {
	...
	if (Number(drawerWidth.split('px')[0]) < 250 && diff < 0) {
		...
		diff > 0 &&
		explorerWindowEl.current &&
		Number(drawerWidth.split('px')[0]) +
			explorerWindowEl.current.previousElementSibling.getBoundingClientRect()
				.width +
			10 >=
			explorerWindowEl.current.parentElement.parentElement.getBoundingClientRect()
				.width
	) {
		drawerWidth = `${
			explorerWindowEl.current.parentElement.parentElement.getBoundingClientRect()
