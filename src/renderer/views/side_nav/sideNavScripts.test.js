...
describe('function handleDrawerToggle:', () => {
	it("expect function to return {drawerWidth: '250px'} when props.drawerWidth is falsy:", () => {
		[undefined, null, '', false, 0].forEach(arg => {
			expect(
				sideNavScripts.handleDrawerToggle({ drawerWidth: arg })
					.drawerWidth,
			).toBe('250px');
		});
	});
...
describe('function handleTerminalToggle:', () => {
	it("expect function to return {terminalHeight: '468px'} when props.terminalHeight is falsy:", () => {
		[undefined, null, '', false, 0].forEach(arg => {
			expect(
				sideNavScripts.handleTerminalToggle({ terminalHeight: arg })
					.terminalHeight,
			).toBe('468px');
		});
	});
...
describe('function getSettingsInitialValues:', () => {
	it('expect all properties of getSettingsInitialValues return value to be undefined:', () => {
		...
		Object.keys(response).forEach(prop => {
			if (prop === 'font_size') {
				expect(response[prop]).toBe(NaN); // eslint-disable-line jest/no-conditional-expect
			} else {
				expect(response[prop]).toBe(undefined); // eslint-disable-line jest/no-conditional-expect
			}
		});
	});
	...
	it('expect return value to be properly transformed:', () => {
		...
		expect(response.prefers_terminal_view).toBe(
			settings.prefersTerminalView,
		);
		expect(response.font_size).toBe(
