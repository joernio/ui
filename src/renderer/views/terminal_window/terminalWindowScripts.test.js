...
describe('function handleTerminalMaximizeToggle:', () => {
	it('expect function to return { isMaximized: true } when arg is falsy:', () => {
		[undefined, null, '', false, 0].forEach(arg => {
			expect(TWS.handleTerminalMaximizeToggle(arg).isMaximized).toBe(
				true,
			);
		});
	});
...
describe('function handleEmptyWorkspace:', () => {
	it('expect function to return empty object:', () => {
		[undefined, null, '', false, 0].forEach(arg => {
			expect(
				Object.keys(TWS.handleEmptyWorkspace(arg, arg)),
			).toHaveLength(0);
		});
	});
	...
	it('expect function to return {isMaximized: true}:', () => {
		expect(
			TWS.handleEmptyWorkspace({ projects: {} }, { projects: {} })
				.isMaximized,
		).toBe(true);
	});
...
describe('function handleEnter:', () => {
	...
	let updateCursorPosition;
	let termWriteLn;
	let constructOutputToWrite;
	let handleWriteToCircuitUIResponse;
	let handleWriteToCircuitUIInput;
	...
	beforeEach(() => {
		...
		getPrev = jest
			...
			.mockImplementation(
				({ prev_queries }) =>
					prev_queries[
						Object.keys(prev_queries)[
							Object.keys(prev_queries).length - 1
						]
					],
			);
		rotatePrev = jest
			.spyOn(TWS, 'rotatePrev')
	...
	beforeEach(() => {
		...
		getNext = jest
			...
			.mockImplementation(
				({ next_queries }) =>
					next_queries[
						Object.keys(next_queries)[
							Object.keys(next_queries).length - 1
						]
					],
			);
		rotateNext = jest
			.spyOn(TWS, 'rotateNext')
			.mockImplementation(obj => obj);
		updateData = jest.spyOn(TWS, 'updateData').mockImplementation(() => {});
		updateCursorPosition = jest
			.spyOn(TWS, 'updateCursorPosition')
...
describe('function handleWriteScriptQuery 2:', () => {
	let updateData;
	let updateCursorPosition;
	let termWriteLn;
	let constructOutputToWrite;
...
describe('function initXTerm:', () => {
	...
	it('expect function to initialize Terminal, call certain functions and return the Terminal object', async () => {
		const shellprompt =
			TV.carriageReturn + TV.newLine + TV.cpgDefaultPrompt;
