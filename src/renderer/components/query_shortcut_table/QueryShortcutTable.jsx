...
function QueryShortcutTable(props) {
  ...
  const { hover, containerWidth } = state;
  ...
  useEffect(() => {
    const callback = () => handleSetState(handleResize(window, drawerWidth));
        ...
        >
          <div className={classes.frame0Style}>
            <div className={classes.contentStyle}>
              <div className={clsx(classes.headingStyle, 'heading')}>
                <div>Edit</div>
              </div>
                        ...
                        <Icon
                          ...
                          onClick={() =>
                            props.handleSetState(
                              openDialog(keybindings[idx], queryShortcuts),
                            )
                          }
                        />
                      </div>
          ...
          <div className={classes.frame1Style}>
            <div className={classes.content}>
              <div className={classes.headingStyle}>{queries[0]}</div>
              <div className={classes.innerContentStyle}>
          ...
          <div ref={refs.frame2Ref} className={classes.frame2Style}>
            <div className={classes.content}>
              <div className={classes.headingStyle}>{keybindings[0]}</div>
              <div className={classes.innerContentStyle}>
                <div
                  onMouseDown={() => handleSetState(handleMouseDown('frame2'))}
                ...
                {keybindings.map((item, idx) =>
                  idx !== 0 ? (
                    ...
                    >
                      {parseKeyBinding(item).map((value, idx) =>
                        value === ' ' || value === '+' ? (
                          <span
                            key={idx}
                            className={classes.tagSeparationStyle}
                          >
                            {value}
                          </span>
                        ) : (
                          <Tag className={classes.keybindingStyle} key={idx}>
                            {value}
                          </Tag>
                        ),
                      )}
          ...
          <div ref={refs.frame3Ref} className={classes.frame3Style}>
            <div className={classes.content}>
              <div className={classes.headingStyle}>{behaviours[0]}</div>
              <div className={classes.innerContentStyle}>
                <div
                  onMouseDown={() => handleSetState(handleMouseDown('frame3'))}
                  className={classes.dragStyle}
                ></div>
          ...
          <div ref={refs.frame4Ref} className={classes.frame4Style}>
            <div className={classes.contentStyle}>
              <div className={classes.headingStyle}>{backgrounds[0]}</div>
              <div className={classes.innerContentStyle}>
                <div
                  onMouseDown={() => handleSetState(handleMouseDown('frame4'))}
                  className={classes.dragStyle}
                ></div>
